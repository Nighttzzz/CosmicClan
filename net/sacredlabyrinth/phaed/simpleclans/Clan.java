...
public class Clan implements Serializable, Comparable<Clan> {
    ...
    public void displayBb(Player player) {
        if (isVerified()) {
        	...
        	player.sendMessage("");
        	messageCenter.sendCenteredMessage(player, "§7" + cp.getClan().getTag() + " - MOTD");
        	player.sendMessage("");
            ...
            for (String msg : bb) {
                ChatBlock.sendMessage(player, "§8* " + SimpleClans.getInstance().getSettingsManager().getBbColor() + Helper.parseColors(ChatColor.GRAY + msg));
            }
            player.sendMessage("");
        }
    }
